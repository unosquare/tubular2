...
describe('grid pager info', () => {
    ...
    it('should show text in accordance to numbered of filter rows and current results-page',() => {
        expect(gridPagerInfo.first().getText()).toEqual('Showing 1 to 10 of 49 records');
        paginator.get(7).$$('a').click();
        pageSizeSelector.$('[value="20"]').click();
        expect(gridPagerInfo.first().getText()).toEqual('Showing 21 to 40 of 49 records');
        paginator.get(5).$$('a').click();
        expect(gridPagerInfo.first().getText()).toEqual('Showing 41 to 60 of 49 records');
    });

    it('should show count in footer', () => {
        gridSearchInput.sendKeys('a');
        expect(gridPagerInfo.first().getText()).toContain('(Filtered from 49 total records)');
    });
});