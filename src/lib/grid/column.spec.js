...
describe('Column Model', () => {
    let columnModel;
    beforeAll(() => {
        columnModel = new column_1.ColumnModel('mock', false, false);
    });
    ...
    describe('get input type', () => {
        it('should be number', () => {
            columnModel.filterMode = column_1.ColumnFilterMode.Number;
            expect(columnModel.getInputType()).toBe('number');
        });
        it('should be date', () => {
            columnModel.filterMode = column_1.ColumnFilterMode.Date;
            expect(columnModel.getInputType()).toBe('date');
        });
        it('should be datetime-local', () => {
            columnModel.filterMode = column_1.ColumnFilterMode.DateTime;
            expect(columnModel.getInputType()).toBe('datetime-local');
        });
        it('should be text', () => {
            columnModel.filterMode = column_1.ColumnFilterMode.None;
            expect(columnModel.getInputType()).toBe('text');
            columnModel.filterMode = column_1.ColumnFilterMode.String;
            expect(columnModel.getInputType()).toBe('text');
            columnModel.filterMode = column_1.ColumnFilterMode.Boolean;
            expect(columnModel.getInputType()).toBe('text');
        });
    });
